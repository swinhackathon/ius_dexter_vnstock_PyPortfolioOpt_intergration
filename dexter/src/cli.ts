...
function summarizeStockAdvisory(parsed: Record<string, unknown>): string {
  const data = parsed.data as Record<string, unknown> | undefined;
  if (!data || typeof data !== 'object') {
    return 'Received advisory payload';
  }

  const keys = Object.keys(data).filter((key) => !key.startsWith('_'));
  const base = `Received ${keys.length} fields`;

  const auditTrail = data.audit_trail as { calls?: unknown[] } | undefined;
  const calls = Array.isArray(auditTrail?.calls) ? auditTrail.calls : [];
  if (calls.length === 0) {
    return base;
  }

  const visibleCalls = calls.slice(0, 8).map((call) => {
    const asCall = call as Record<string, unknown>;
    const tool = typeof asCall.tool === 'string' ? asCall.tool : 'internal_call';
    const symbol = typeof asCall.symbol === 'string' ? asCall.symbol : '';
    const status = typeof asCall.status === 'string' ? asCall.status.toUpperCase() : 'OK';
    const detail = typeof asCall.detail === 'string' ? truncateAtWord(asCall.detail, 55) : '';
    const symbolPart = symbol ? `(ticker=${symbol})` : '';
    const detailPart = detail ? ` - ${detail}` : '';
    return `- ${formatSubToolName(tool)}${symbolPart} [${status}]${detailPart}`;
  });

  const hiddenCount = calls.length - visibleCalls.length;
  const hidden = hiddenCount > 0 ? `\n... +${hiddenCount} more internal calls` : '';
  return `${base}\nInternal advisory calls:\n${visibleCalls.join('\n')}${hidden}`;
}
  ...
  try {
    ...
    if (parsed.data) {
      ...
      if (typeof parsed.data === 'object') {
        ...
        if (tool === 'stock_advisory') {
          return summarizeStockAdvisory(parsed);
        }
